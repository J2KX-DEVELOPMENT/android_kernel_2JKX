		...
		struct fimc_is_actuator *actuator)
{
	...
	fimc_is_sec_get_sysfs_finfo(&sysfs_finfo);
	temp_step= sysfs_finfo->af_cal_pan - DEF_DW9808_AF_PAN;
	if(temp_step < 0 ){
		if(-temp_step > DEF_DW9808_OFFSET_POSITION_MAX)
			offset_step = -DEF_DW9808_OFFSET_POSITION_MAX;
		else
	...
	if (init_step > 0) {
		...
		ret = sensor_dw9808_write_position(client, DEF_DW9808_FIRST_POSITION + offset_step);
		if (ret < 0)
			goto p_err;

		mdelay(DEF_DW9808_FIRST_DELAY);

		ret = sensor_dw9808_write_position(client, DEF_DW9808_SECOND_POSITION + offset_step);
		if (ret < 0)
			goto p_err;

		mdelay(DEF_DW9808_SECOND_DELAY);

		actuator->position = DEF_DW9808_SECOND_POSITION + offset_step;

		dbg_sensor("initial position %d, %d setting\n", DEF_DW9808_FIRST_POSITION + offset_step, DEF_DW9808_SECOND_POSITION + offset_step);
	}
		...
		const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_act_id(device, ACTUATOR_NAME_DW9808);
	if (!sensor_peri) {
