...
int sensor_4e6_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	/* set reset :4e6 sensor must reset before mode change */
...
int sensor_4e6_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	dbg_sensor("[MOD:D:%d] %s, input_again = %d/%d, analog_gain(%d/%d)\n",
			cis->id, __func__, again->short_val, again->long_val, short_gain, long_gain);
