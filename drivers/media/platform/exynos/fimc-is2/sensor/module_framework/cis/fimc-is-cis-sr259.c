...
int sensor_sr259_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	dbg_sensor("[MOD:D:%d] %s, vt_pic_clk_freq_mhz (%d), line_length_pck(%d)\n", cis->id, __func__,
...
int sensor_sr259_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	...
	*max_expo = max_integration_time;
	...
	dbg_sensor("[%s] max integration time %d, max coarse integration %d\n",
...
int sensor_sr259_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	...
	dbg_sensor("[MOD:D:%d] %s, cis->min_fps(%d), cis->max_fps(%d)\n",
	...
	if (cis->min_fps == cis->max_fps) {
		switch (cis->min_fps) {
			...
			case SR259_30_FIXED_FPS :
				...
				break;
			default :
				dbg_sensor("[MOD:D:%d] %s, need not additional setting for fixed FPS\n", cis->id, __func__);
				break;
		}
	} else {
		if (is_fixed_fps) {
...
int cis_sr259_probe(struct i2c_client *client,
...
static int cis_sr259_remove(struct i2c_client *client)
{
	int ret = 0;
	return ret;
}

static const struct of_device_id exynos_fimc_is_cis_sr259_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-sr259",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_sr259_match);

static const struct i2c_device_id cis_sr259_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver cis_sr259_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_sr259_match
	},
	.probe	= cis_sr259_probe,
	.remove	= cis_sr259_remove,
	.id_table = cis_sr259_idt
};
module_i2c_driver(cis_sr259_driver);
