	...
	do {
		...
		if (!busy && mmc_test_busy(&cmd)) {
			...
			if (test->card->host->caps & MMC_CAP_WAIT_WHILE_BUSY)
				pr_no_info("%s: Warning: Host did not "
				...
				struct timespec *ts1, struct timespec *ts2)
{
	...
	pr_no_info("%s: Transfer of %u sectors (%u%s KiB) took %lu.%09lu "
				    ...
				    struct timespec *ts2)
{
	...
	pr_no_info("%s: Transfer of %u x %u sectors (%u x %u%s KiB) took "
...
{
	pr_no_info("%s: Highmem not configured - test skipped\n",
	       mmc_hostname(test->card->host));
	return 0;
}
			     ...
			     int max_scatter, int min_sg_len)
{
	...
	if (err)
		pr_no_info("%s: Failed to map sg list\n",
		       mmc_hostname(test->card->host));
	return err;
}
				     ...
				     struct mmc_test_multiple_rw *rw)
{
	...
	if (rw->do_nonblock_req &&
		...
		pr_no_info("error: only one of pre/post is defined\n");
		return -EINVAL;
	}
...
static void mmc_test_run(struct mmc_test_card *test, int testcase)
{
	...
	pr_no_info("%s: Starting tests of card %s...\n",
	...
	for (i = 0;i < ARRAY_SIZE(mmc_test_cases);i++) {
		...
		switch (ret) {
		case RESULT_OK:
			pr_no_info("%s: Result: OK\n",
				mmc_hostname(test->card->host));
			break;
		case RESULT_FAIL:
			pr_no_info("%s: Result: FAILED\n",
				mmc_hostname(test->card->host));
			break;
		case RESULT_UNSUP_HOST:
			pr_no_info("%s: Result: UNSUPPORTED "
				"(by host)\n",
				mmc_hostname(test->card->host));
			break;
