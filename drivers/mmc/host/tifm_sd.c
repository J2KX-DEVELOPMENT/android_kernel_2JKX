...
static void tifm_sd_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (host->req) {
		pr_no_err("%s : unfinished request detected\n",
		       dev_name(&sock->dev));
	...
	if (r_data) {
		...
		if (host->no_dma) {
			...
			if(1 != tifm_map_sg(sock, &host->bounce_buf, 1,
					    ...
					    ? PCI_DMA_TODEVICE
					    : PCI_DMA_FROMDEVICE)) {
				pr_no_err("%s : scatterlist map failed\n",
				       dev_name(&sock->dev));
...
static void tifm_sd_end_cmd(unsigned long data)
{
	...
	if (!mrq) {
		pr_no_err(" %s : no request to complete?\n",
		       dev_name(&sock->dev));
		spin_unlock_irqrestore(&sock->lock, flags);
		return;
	}
...
static void tifm_sd_abort(unsigned long data)
{
	...
	pr_no_err("%s : card failed to respond for a long period of time "
	       "(%x, %x)\n",
...
static int tifm_sd_initialize_host(struct tifm_sd *host)
{
	...
	if (rc) {
		pr_no_err("%s : controller failed to reset\n",
		       dev_name(&sock->dev));
		return -ENODEV;
	}
	...
	if (rc) {
		pr_no_err("%s : card not ready - probe failed on initialization\n",
		       dev_name(&sock->dev));
		return -ENODEV;
	}
