/* linux/drivers/video/backlight/s6d78a0_qhd_gpp_go_mipi_lcd.c
 *
...
#include <linux/pwm.h>
#include "s6d78a0_qhd_gpp_go_param.h"
#include "dd.h"
...
void ktd3102_set_bl_level(struct lcd_info *lcd)
{
	unsigned long flags;
...
static int dsim_panel_set_brightness(struct lcd_info *lcd, unsigned int force)
{
	int ret = 0;
	mutex_lock(&lcd->lock);
	...
	lcd->bl = brightness_table[lcd->brightness];

	dev_info(&lcd->ld->dev, "%s: platform BL : %d tune_level(%d)\n", __func__, lcd->brightness, lcd->bl);
...
static int s6d78a0_init(struct lcd_info *lcd)
{
	...
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_SLEEP_OUT_1\n", __func__);
		goto init_exit;
	}
...
static int ktd3102_probe(struct lcd_info *lcd, struct device_node *parent)
{
	...
	if (lcd->bl_pin_ctrl < 0) {
		dev_err(&lcd->ld->dev, "%s, failed to parse dt\n", __func__);
		return -EINVAL;
	}

	ret = gpio_request(lcd->bl_pin_ctrl, "BL_CTRL");
	if (unlikely(ret < 0)) {
		dev_err(&lcd->ld->dev, "%s, request gpio(%d) failed\n", __func__, lcd->bl_pin_ctrl);
