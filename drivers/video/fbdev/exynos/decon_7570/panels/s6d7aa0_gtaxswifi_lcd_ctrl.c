...
#include "s6d7aa0_gtaxswifi_param.h"
#include "dd.h"

#if defined(CONFIG_EXYNOS_DECON_MDNIE_LITE)
#include "mdnie.h"
#include "mdnie_lite_table_gtaxswifi.h"
#endif
...
static int s6d7aa0_init(struct lcd_info *lcd)
{
	...
	ret = dsim_write_hl_data(lcd, SEQ_35_PARAM, ARRAY_SIZE(SEQ_35_PARAM));
	if (ret < 0) {
		dev_err(&lcd->ld->dev, "%s: failed to write CMD : SEQ_35_PARAM\n", __func__);
		goto init_exit;
	}
